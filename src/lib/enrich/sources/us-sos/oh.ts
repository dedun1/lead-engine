...
const SEARCH = 'https://businesssearch.ohiosos.gov/';

export async function searchOh(lead: EnrichLead): Promise<Partial<EnrichedFields>> {
  ...
  try {
    ...
    const patch: Partial<EnrichedFields> = {
      business_registration: { registry: 'us_sos_oh', registry_id: lead.business_name, officers },
    };
    const owner = officerOwnerCandidate(officers);
    if (owner) patch.owner_name = owner;
    return successLog('us_sos_oh', patch, Date.now() - start);
  } catch (error) {
    return skipLog('us_sos_oh', error instanceof Error ? error.message : 'fail', Date.now() - start);
  }
}
